...
/// /    @BeforeAll /    public void setUp() { /        objectMapper = new ObjectMapper(); /        requestDto =
/// ProductCategoryRequestDto.builder().categoryName("요리류").build(); /    }
//
//    @Test
//    @DisplayName("상품 카테고리 등록 성공 테스트")
//    public void createProductCategory_ShouldReturnCreated() throws Exception {
//        //given
//        ProductCategoryResponseDto responseDto = ProductCategoryResponseDto.builder().id(UUID.randomUUID()).name("면류").build();
//
//        given(productCategoryService.createProductCategory(any(ProductCategoryRequestDto.class))).willReturn(responseDto);
//
//        //when
...
//                    .authorizeHttpRequests((authorizeHttpRequests)->authorizeHttpRequests.anyRequest().permitAll());  // 모든 요청을 허용
//            return http.build();
//        }
//    }
//}
